...
function createAudioResponseService({
  ...
  async function sendAudioReply({ text, to, from, publicBaseUrl }) {
    console.log("Audio output requested.");

    if (!config.enableAudioOutput) {
      console.log("Audio output skipped: ENABLE_AUDIO_OUTPUT is disabled.");
      return;
    }

    if (!twilioClient) {
      console.warn("Audio output skipped: Twilio REST client is not configured.");
      return;
    }

    if (!sarvamService.isConfigured) {
      console.warn("Audio output skipped: Sarvam API key is not configured.");
      return;
    }

    if (!publicBaseUrl) {
      console.warn(
        "Audio output skipped: PUBLIC_BASE_URL is missing or the request host is not publicly reachable."
      );
      return;
    }

    if (!from || !to) {
      console.warn("Audio output skipped: missing WhatsApp sender or recipient.");
      return;
    }

    const languageCode = detectSarvamLanguageCode(text);
    if (!languageCode) {
      console.warn("Audio output skipped: unsupported language for Sarvam TTS.");
      return;
    }

    console.log(`Audio output language detected: ${languageCode}`);

    const chunks = splitTextForTts(text, 2200);
    console.log(`Audio output split into ${chunks.length} chunk(s).`);
